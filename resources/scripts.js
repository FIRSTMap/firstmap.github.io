...
function createCompetitionMarker(type, competitionEntry) {
  if (competitionEntry) {
    var position = {
      lat:
        type == "regional" ? competitionEntry[1].lat : competitionEntry[2].lat,
      lng:
        type == "regional" ? competitionEntry[1].lng : competitionEntry[2].lng
    };
...
function openTeamInfo(num, marker) {
  ...
  req.onreadystatechange = function() {
    if (req.readyState === 4 && req.status === 200) {
      ...
      try {
        var oldInfoWindow = document.getElementsByClassName("gm-style-iw")[0];
        oldInfoWindow.parentNode.parentNode.removeChild(
          oldInfoWindow.parentNode
        );
      } catch (e) {}
